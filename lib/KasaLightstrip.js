...
class KasaLightstrip {
    constructor(log, config, api, debug, uuid, device) {
        ...
        this.deviceService = this.device.getService(this.api.hap.Service.Lightbulb) || this.device.addService(this.api.hap.Service.Lightbulb);
    ...
    updateAllCharacteristics() {
                ...
                if(!this.checkingOn) {
                    ...
                    exec(`kasa --host ${this.ip} --lightstrip`, (err, stdout, stderr) => {
                        if(err) {
                            ...
                            this.deviceService.updateCharacteristic(this.api.hap.Characteristic.On, this.onStatus);   //keep in sync in case the light was turned on elsewhere
                        }
                        this.checkingOn = false;
                    });
                }
        ...
        //Brightness = int
        this.deviceService.getCharacteristic(this.api.hap.Characteristic.Brightness)
            .onSet(async (state) => {
        ...
        //Hue = int
        this.deviceService.getCharacteristic(this.api.hap.Characteristic.Hue)
            .onSet(async (state) => {
                this.tempHue = state;
        ...
        //Saturation = int
        this.deviceService.getCharacteristic(this.api.hap.Characteristic.Saturation)
            .onSet(async (state) => {
        ...
        if(this.tempHue != undefined && this.tempSaturation != undefined) {
            ...
            exec(`kasa --host ${this.ip} --lightstrip hsv ${hue} ${saturation} ${this.brightness}`, (err, stdout, stderr) => {
                ...
                if(this.onStatus == false) {
                    ...
                    this.onStatus = true;
                    this.deviceService.updateCharacteristic(this.api.hap.Characteristic.On, this.onStatus);
                }
            });
            this.tempHue = undefined;
            this.tempSaturation = undefined;
