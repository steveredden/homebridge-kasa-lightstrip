            ...
            if(this.config.accessories && Array.isArray(this.config.accessories)) {
                for (let lightstrip of this.config.accessories) {
                    ...
                    for (let effect of lightstrip.effects) {
                        ...
                        if(effect == "CustomEffects") uniqueEffect = effect.name
                        else uniqueEffect = effect

                        const uuid = this.api.hap.uuid.generate('homebridge-kasa-lightstrip' + lightstrip.name + lightstrip.ip + uniqueEffect);
                        let accessory = this.accessories.find(accessory => accessory.UUID === uuid)
